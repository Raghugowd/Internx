...
const AdminInternships = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingInternship) {
        await axios.put(`${API_BASE_URL}/admin/internships/${editingInternship._id}`, internshipData);
        toast.success('Internship updated successfully!');
      } else {
        await axios.post(`${API_BASE_URL}/admin/internships`, internshipData);
  ...
  const handleDelete = async (internshipId: string) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/admin/internships/${internshipId}`);
  ...
  const handleBulkUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/admin/internships/bulk-upload`, formData, {
        headers: {
      ...
      toast.error(error.response?.data?.message || 'Failed to upload internships');
    } finally {
      setUploading(false);
    }
  };
  ...
  const toggleInternshipStatus = async (internshipId: string, currentStatus: boolean) => {
    try {
      await axios.put(`${API_BASE_URL}/admin/internships/${internshipId}`, {
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Internship Management</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-700/50">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Internship Details</th>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-gray-400">
                      No internships found. Create your first internship posting!
                    </td>
                  </tr>
                ) : (
                  internships.map((internship) => (
                    <tr key={internship._id} className="hover:bg-gray-700/30 transition-colors">
                      <td className="px-6 py-4">
                        <div>
                          <div className="text-white font-medium">{internship.title}</div>
                          <div className="text-gray-400 text-sm">{internship.domain} • {internship.type}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-gray-300">{internship.company}</td>
                      <td className="px-6 py-4 text-gray-300">{internship.location}</td>
                      <td className="px-6 py-4 text-gray-300">₹{internship.salary.toLocaleString()}</td>
                      <td className="px-6 py-4">
                        <button
            ...
            <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Internship Title *</label>
                    <input
                      type="text"
                      name="title"
        ...
        {showBulkModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-md">
              <h2 className="text-2xl font-bold text-white mb-6">Bulk Upload Internships</h2>
              ...
              <form onSubmit={handleBulkUpload} className="space-y-4">
                ...
                <div className="flex space-x-4 pt-4">
                  ...
                  >
                    {uploading ? 'Uploading...' : 'Upload Internships'}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                ...
                <table className="w-full">
                  <thead className="bg-gray-700/50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase">Internships Created</th>
                    ...
                    {excelFiles.length === 0 ? (
                      ...
                      excelFiles.map((file) => (
                        <tr key={file._id} className="hover:bg-gray-700/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-gray-300">{file.internshipsCreated}</td>
